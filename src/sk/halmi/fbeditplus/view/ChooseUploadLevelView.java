...
import sk.halmi.fbeditplus.overview.ChooseUploadLevelActivity;
import android.content.Context;
import android.graphics.Paint.Style;
import android.util.AttributeSet;

public class ChooseUploadLevelView extends OverviewView {

	public ChooseUploadLevelView(Context context, AttributeSet attrs) {
		super(context, attrs);
	}
	...
	protected void dimmSquare(float x, float y) {
		...
		if (whichLevelClicked <= levels.length/levelSize) {
			...
			if (y < 3*mHeight/4) ((ChooseUploadLevelActivity)mContext).showActionsDialog(whichLevelClicked);
		}
	}
